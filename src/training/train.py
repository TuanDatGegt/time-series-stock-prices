...
def _validate_sequence_set(
	...
	if y.shape != (len(X), 1):
		raise ValueError(f"Incompatible target dimensions: expected ({len(X)}, 1), got {y.shape}")
...
def run_training(
	...
	logger = logger or get_logger(level=resolved.get("logging", {}).get("level", "INFO"))
	...
	missing = [column for column in feature_cols + [target_col] if column not in featured.columns]
	if missing:
		raise ValueError(f"Configured columns missing after feature creation: {missing}")
	...
	if model_name in {"lstm", "gru"}:
		train_X, train_y = build_sequences(scaled_train, lookback, horizon, feature_cols, target_col)
		val_X, val_y = build_sequences(scaled_val, lookback, horizon, feature_cols, target_col)
		test_X, test_y = build_sequences(scaled_test, lookback, horizon, feature_cols, target_col)
		...
		model, model_config_object = create_model(model_name, len(feature_cols), model_config)
	...
	logger.info("Training complete: symbol=%s model=%s checkpoint=%s", symbol, model_name, checkpoint_path)
